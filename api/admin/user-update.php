...
if (isset($json["secret_key"])) {
	...
	$idStudent = trim(mysqli_real_escape_string($mysqli,$json["idStudent"]));
	$idUser = trim(mysqli_real_escape_string($mysqli,$json["idUser"]));
    $sql = "UPDATE students SET studentid = '$studentid', lastname = '$lastname', firstname = '$firstname', address = '$address', contact = '$contact', birthday = '$birthday', email = '$email', course = '$course', year = '$year', section = '$section' WHERE id=$idStudent";
    if ($action == "delete") {
    	$sql = "DELETE FROM students WHERE id=$idStudent";
    }
    try {
    ...
    $sql = "UPDATE users SET userid = '$studentid', role = '$role' WHERE id=$idUser";
    if (strlen($password) > 0) {
    	$sql = "UPDATE users SET userid = '$studentid', password = '$password', role = '$role' WHERE id=$idUser";
    }
    if ($action == "delete") {
